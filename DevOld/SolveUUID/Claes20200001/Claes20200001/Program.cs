...
namespace Charlotte
{
	class Program
	{
		...
		private void Main4(ArgsReader ar)
		{
			...
			// ダブルクリックで起動したときコンソールが見えるように少し待つ
			Thread.Sleep(500);
		}
		...
		private void SolveUUID()
		{
			...
			this.CollectUUIDPosition("C:\\Dev", "Program.c", SCommon.ENCODING_SJIS, "#define APP_IDENT \"", "\" // アプリ毎に変更する。");
			this.CollectUUIDPosition("C:\\Dev", "Program.js", SCommon.ENCODING_SJIS, "var APP_IDENT = \"", "\"; // アプリ毎に変更する。");
		...
		private void SolveUUIDCollision()
		{
			...
			foreach (UUIDPositionInfo up in this.UUIDPositions)
			{
				if (up.UUIDChanged)
				{
					...
					// なんちゃってログ出力
					File.AppendAllLines(@"C:\temp\SolveUUID_ChangedFiles.txt", new string[] { "[" + DateTime.Now + "] " + up.FilePath }, Encoding.UTF8);
